...
def favorite(num)
  ...
  client.search("#bonjour_monde", result_type: "recent").take(num).collect do |tweet| 
    client.favorite!(tweet) 
    sleep(2)
  end
end

def follow_twitter(num)
  client = login_twitter
  client.search("#bonjour_monde -rt", result_type: "recent").take(num).collect do |tweet| 
...
def like_and_follow
  client_stream = streaming_log_twitter
  client = login_twitter
  topics = ["#bonjour_monde"]
  client_stream.filter(track: topics[0]) do |object|
    client.favorite!(object)
    client.follow(object.user.screen_name)
    puts "#{object.user.screen_name} a tweeté #{object.text}"
  end
end
...
def send_email
  array = random_sample
  client = login_twitter
  array.each{|handle| client.update("Hello #{handle} je te souhaite une bonne journée! #bonjour_monde")} 
end
